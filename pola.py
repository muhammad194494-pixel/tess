...
def predict_with_ci(model, last_multipliers, n_iter=50):
    """
    Menggunakan Monte Carlo Dropout untuk mendapat confidence interval.
    Asumsi: model memiliki dropout layer yang tetap aktif saat inference.
    """
    if len(last_multipliers) < SEQ_LEN:
        print(f"[ERROR] Butuh {SEQ_LEN} data terakhir")
        return None, None, None
    
    # Preprocess last_multipliers (winsorize dulu? tapi kita tidak tahu batas global)
    # Gunakan winsorize dengan batas yang sama (0.01, 0.99) berdasarkan data training tersimpan
    # Untuk sederhana, clip ke [1, 50] saja
    last_clipped = np.clip(last_multipliers, 1, 50)
    
    # Buat fitur untuk sequence terakhir
    # Kita perlu create_features dari sequence last_clipped (butuh array panjang minimal SEQ_LEN)
    # Cara: buat array palsu dengan data terakhir lalu di-feature
    ...
    # MC Dropout: aktifkan training mode untuk dropout
    preds = []
    for _ in range(n_iter):
        pred_log = model(X_last, training=True).numpy()[0,0]
        preds.append(pred_log)
    pred_log_mean = np.mean(preds)
    pred_log_std = np.std(preds)
